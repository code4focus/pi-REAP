...
const config = (overrides: Partial<{ enabled: boolean; mode: "shadow" | "enforce"; ambiguousEffort: "high" | "xhigh"; failureEffort: "high" | "xhigh"; showStatus: boolean; notifyOnEscalation: boolean }> = {}) => ({
  enabled: overrides.enabled ?? true, mode: overrides.mode ?? "enforce", ambiguousEffort: overrides.ambiguousEffort ?? "high", failureEffort: overrides.failureEffort ?? "xhigh",
  telemetry: { enabled: true, includePromptText: false, directory: "synthetic" }, ui: { showStatus: overrides.showStatus ?? true, notifyOnEscalation: overrides.notifyOnEscalation ?? false },
});
...
describe("production configuration seam", () => {
  ...
  it("keeps shadow requests unchanged and enforce patches them", async () => {
  ...
  it("applies configured failure floors through production handlers", async () => { const h = new ExtensionHarness(); await createExtension({ load: async () => config({ failureEffort: "high" }) })(h.api()); h.start(); run(h, "What is JSON?"); h.error(); expect(run(h, "continue")?.reasoning?.effort).toBe("high"); });
});

...
class InMemoryPiSession {
  ...
  private readonly context: HarnessContext = { cwd: process.cwd(), model: { id: "synthetic-evaluation", provider: "openai", api: "openai-responses", reasoning: true }, sessionManager: { getSessionId: () => this.sessionId }, ui: { setStatus: (key, text) => this.statuses.set(key, text), notify: () => undefined } };
  ...
  async setMode(mode: "shadow" | "enforce"): Promise<void> { await this.commands.get("effort")?.handler(mode, this.lifecycleContext); }
  ...
  recommendedEffort(): AutomaticEffort { const status = this.statuses.get("pi-reap") ?? ""; const match = /effort:(?:auto|low|medium|high|xhigh|max) → (low|medium|high|xhigh)/.exec(status); if (!match) throw new Error("production extension did not expose a shadow recommendation"); return match[1] as AutomaticEffort; }
...
export class PiSessionExecutor implements EvaluationExecutor {
  ...
  static async create(options: ExtensionOptions, controlModel: ProviderModel = defaultControlModel): Promise<PiSessionExecutor> {
    const executor = new PiSessionExecutor(controlModel);
    // Evaluation observes shadow recommendations through the production status
    // surface; this does not enable enforcement or alter provider payloads.
    const load = options.load;
    await createExtension({ ...options, load: async () => {
      const config = await load!();
      return { ...config, ui: { ...config.ui, showStatus: true } };
    } })(executor.session.asExtensionApi());
  ...
  async execute(task: CorpusTask, request: ExecutionRequest): Promise<ExecutionResult> {
    if (request.mode === "fixed-xhigh" || request.mode === "fixed-high" || request.mode === "candidate") {
    ...
    await this.session.setMode(request.mode === "policy-shadow" ? "shadow" : "enforce");
    const payload = this.session.run(task.description);
    // Shadow returns no patch.  Its recommendation is observed through the
    // production extension status rather than being represented as applied.
    const effort = request.mode === "policy-shadow" ? this.session.recommendedEffort() : selectedEffort(payload);

...
export const acceptedBaseSha256 = "cbdbf256286ee7fb3d05e52ac7d702dfc0838ec6";
...
export interface SanitizedLiveRun {
  ...
  readonly providerRequests: number; readonly toolRounds: number; readonly retries: number; readonly usage: UsageMetrics; readonly latencyMs: number; readonly effectiveCostMicros: number; readonly accepted: boolean; readonly criticalFailure: boolean;
}
...
export function validateTrustedLiveAcceptance(value: unknown, suppliedAttestation: unknown): TrustedLiveAcceptanceReceipt {
  ...
  if (attestation.witnessHash !== pinnedWitnessHash || attestation.reviewerNonce !== pinnedReviewerNonce || attestation.planSha256 !== frozenPlanSha256 || attestation.acceptedBaseSha256 !== acceptedBaseSha256 || attestation.sourceFingerprint !== expectedSourceFingerprint || attestation.extensionBuildFingerprint !== expectedExtensionBuildFingerprint) fail("trusted attestation has an unpinned binding");
...
function validateUsage(value: unknown): UsageMetrics { const usage = record(value, "usage"); exactKeys(usage, usageKeys, "usage"); for (const key of usageKeys) integer(usage[key], key, 0); const typed = usage as unknown as UsageMetrics; if (typed.inputTokens !== typed.uncachedInputTokens + typed.cacheReadTokens) fail("input token accounting is inconsistent"); return typed; }
...
function validateCacheCosts(groups: readonly CacheCrossoverGroup[], pricing: TokenPricing): void { for (const group of groups) { hash(cacheControlsFingerprint(group.controls), "cache controls fingerprint"); for (const sample of group.samples) { validateUsage(sample.usage); finite(sample.latencyMs, "cache latency"); effectiveCostMicros(sample.usage, pricing); } } }
...
function validateCeilings(artifact: SanitizedLiveAcceptanceArtifact, attestation: TrustedLiveAttestation): void { const cache = artifact.cacheCrossover.flatMap((group) => group.samples); const all = [...artifact.runs.map((run) => run.usage), ...cache.map((sample) => sample.usage)]; const sums = all.reduce((total, usage) => ({ input: total.input + usage.inputTokens, output: total.output + usage.outputTokens, reasoning: total.reasoning + usage.reasoningTokens }), { input: 0, output: 0, reasoning: 0 }); const requests = artifact.runs.reduce((total, run) => total + run.providerRequests, 0) + cache.reduce((total, sample) => total + sample.providerRequests, 0); const retries = artifact.runs.reduce((total, run) => total + run.retries, 0) + cache.reduce((total, sample) => total + sample.retries, 0); const cost = artifact.runs.reduce((total, run) => total + run.effectiveCostMicros, 0) + cache.reduce((total, sample) => total + effectiveCostMicros(sample.usage, attestation.pricing), 0); if (requests !== 33 || retries !== 0 || sums.input > attestation.ceilings.maxInputTokens || sums.output > attestation.ceilings.maxOutputTokens || sums.reasoning > attestation.ceilings.maxReasoningTokens || cost > attestation.ceilings.maxEffectiveCostMicros) fail("trusted live acceptance exceeds bounded matrix or ceiling"); }
